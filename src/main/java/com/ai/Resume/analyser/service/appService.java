...
public class appService {
    ...
    // TalentRevive: every resume analysis also feeds the HR-side candidate
    // rediscovery pool.
    @Autowired
    private com.ai.Resume.analyser.service.candidateIngestionService candidateIngestionService;
    ...
    public ResponseEntity<?> extract(String roles, MultipartFile file) throws TikaException, IOException, InterruptedException {
        ...
        Content content = Content.builder().parts(Part.fromText(extracted), Part.fromText("You are now an advanced enterprise-grade ATS resume checker, brutally honest career auditor, hiring-manager brain, and growth strategist combined. Your task is to analyze the given resume strictly based on industry-level ATS standards and evaluate it for the specified roles. The evaluation should be moderate to strict (not lenient). A resume should only receive a score between 90 and 100 if it is nearly perfect across all aspects and the content is highly relevant to the specified roles. If any section content is irrelevant to the role, give zero points for that section.\n" +
                ...
                "- Also estimate 'experienceYears': the candidate's total years of professional work experience as a whole number, based strictly on dates/roles stated in the resume. Do not invent experience. Use 0 for entry-level/fresher resumes or when it cannot be determined.\n" +
        ...
        if (resultsDto.getScore() != 0) {
            ...
            previousTable processedData = new previousTable(uname, resultsDto.getScore(), resultsDto.getAtsoptimizationscore(), roles, resultsDto.getExperienceYears(), resultsDto.getPros(), resultsDto.getCons(), resultsDto.getSuggestions());
            ...
            if (usermod != null) {
                ...
                candidateIngestionService.upsertFromAnalysis(uname, usermod.getUsername(), extracted, roles, resultsDto.getExperienceYears());
            }
    ...
    public ResponseEntity<?> lastReport() {
        ...
        if (previousTable != null) {
            ...
            try {
                ...
                resultsDto resultsDto = new resultsDto(previousTable.getScore(), previousTable.getAtsoptimizationscore(), previousTable.getPros(), previousTable.getCons(), previousTable.getSuggestions(), new ArrayList<>(), previousTable.getExperienceYears());
                return ResponseEntity.ok(resultsDto);
            }
            resultsDto resultsDto = new resultsDto(previousTable.getScore(), previousTable.getAtsoptimizationscore(), previousTable.getPros(), previousTable.getCons(), previousTable.getSuggestions(), jobs, previousTable.getExperienceYears());
            return ResponseEntity.ok(resultsDto);
        } else {
