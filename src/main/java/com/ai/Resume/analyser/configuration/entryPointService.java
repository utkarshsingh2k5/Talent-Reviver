...
public class entryPointService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        // TalentRevive: HR users get an additional ROLE_HR authority, gated in securityConfiguration.
        if ("HR".equalsIgnoreCase(user.getRole())) {
            return User.builder().username(user.getEmail()).password(user.getPassword() != null ? user.getPassword() : "").roles("user", "hr").build();
        }
